#include "drv_sensors.h"
#include "iot_i2c.h"
#include "stdint.h"
#include "iot_errno.h"
#include <stddef.h>
#include <stdio.h>
#include <string.h>

#define I2C_HANDLE EI2C0_M2
#define SHT30_I2C_ADDRESS 0x44
#define BH1750_I2C_ADDRESS 0x23

static uint32_t sht30_init(void)
{
    uint32_t ret = 0;
    uint8_t send_data[2] = {0x22, 0x36};
    uint32_t send_len = 2;

    ret = IoTI2cWrite(I2C_HANDLE, SHT30_I2C_ADDRESS, send_data, send_len);
    if (ret != IOT_SUCCESS)
    {
        printf("I2c write failure.\r\n");
        return IOT_FAILURE;
    }

    return IOT_SUCCESS;
}

static uint32_t bh1750_init(void)
{
    uint32_t ret = 0;
    uint8_t send_data[1] = {0x10};
    uint32_t send_len = 1;

    ret = IoTI2cWrite(I2C_HANDLE, BH1750_I2C_ADDRESS, send_data, send_len);
    if (ret != IOT_SUCCESS)
    {
        printf("I2c write failure.\r\n");
        return IOT_FAILURE;
    }

    return IOT_SUCCESS;
}

static float sht30_calc_RH(uint16_t u16sRH)
{
    float humidityRH = 0;


    u16sRH &= ~0x0003;


    humidityRH = (100 * (float)u16sRH / 65535);

    return humidityRH;
}

static float sht30_calc_temperature(uint16_t u16sT)
{
    float temperature = 0;


    u16sT &= ~0x0003;


    temperature = (175 * (float)u16sT / 65535 - 45);

    return temperature;
}

static uint8_t sht30_check_crc(uint8_t *data, uint8_t nbrOfBytes, uint8_t checksum)
{
    uint8_t crc = 0xFF;
    uint8_t bit = 0;
    uint8_t byteCtr ;
    const int16_t POLYNOMIAL = 0x131;


    for(byteCtr = 0; byteCtr < nbrOfBytes; ++byteCtr)
    {
        crc ^= (data[byteCtr]);
        for ( bit = 8; bit > 0; --bit)
        {
            if (crc & 0x80) crc = (crc << 1) ^ POLYNOMIAL;
            else crc = (crc << 1);
        }
    }

    if(crc != checksum)
        return 1;
    else
        return 0;
}

void sht30_read_data(double *temp, double *humi)
{
    static double last_valid_temp = 27.0;
    static double last_valid_humi = 55.0;

    uint8_t data[3];
    uint16_t tmp;
    uint8_t rc;
    uint32_t ret;
    uint8_t temp_valid = 0;
    uint8_t humi_valid = 0;

    uint8_t SHT30_Data_Buffer[6];
    memset(SHT30_Data_Buffer, 0, 6);
    uint8_t send_data[2] = {0xE0, 0x00};

    if (temp != NULL) {
        *temp = last_valid_temp;
    }
    if (humi != NULL) {
        *humi = last_valid_humi;
    }

    uint32_t send_len = 2;
    ret = IoTI2cWrite(I2C_HANDLE, SHT30_I2C_ADDRESS, send_data, send_len);
    if (ret != IOT_SUCCESS) {
        printf("[safe_charge] SHT30 write failed, keep %.1fC %.1f%%\r\n",
            last_valid_temp, last_valid_humi);
        return;
    }

    uint32_t receive_len = 6;
    ret = IoTI2cRead(I2C_HANDLE, SHT30_I2C_ADDRESS, SHT30_Data_Buffer, receive_len);
    if (ret != IOT_SUCCESS) {
        printf("[safe_charge] SHT30 read failed, keep %.1fC %.1f%%\r\n",
            last_valid_temp, last_valid_humi);
        return;
    }


    data[0] = SHT30_Data_Buffer[0];
    data[1] = SHT30_Data_Buffer[1];
    data[2] = SHT30_Data_Buffer[2];
    rc = sht30_check_crc(data, 2, data[2]);
    if(!rc && !(data[0] == 0x00 && data[1] == 0x00))
    {
        tmp = ((uint16_t)data[0] << 8) | data[1];
        double calc_temp = sht30_calc_temperature(tmp);
        if (calc_temp > -20.0 && calc_temp < 90.0) {
            last_valid_temp = calc_temp;
            temp_valid = 1;
            if (temp != NULL) {
                *temp = last_valid_temp;
            }
        }
    }


    data[0] = SHT30_Data_Buffer[3];
    data[1] = SHT30_Data_Buffer[4];
    data[2] = SHT30_Data_Buffer[5];
    rc = sht30_check_crc(data, 2, data[2]);
    if(!rc && !(data[0] == 0x00 && data[1] == 0x00))
    {
        tmp = ((uint16_t)data[0] << 8) | data[1];
        double calc_humi = sht30_calc_RH(tmp);
        if (calc_humi >= 0.0 && calc_humi <= 100.0) {
            last_valid_humi = calc_humi;
            humi_valid = 1;
            if (humi != NULL) {
                *humi = last_valid_humi;
            }
        }
    }

    if (!temp_valid || !humi_valid) {
        printf("[safe_charge] SHT30 invalid data %02x %02x %02x %02x %02x %02x, keep %.1fC %.1f%%\r\n",
            SHT30_Data_Buffer[0], SHT30_Data_Buffer[1], SHT30_Data_Buffer[2],
            SHT30_Data_Buffer[3], SHT30_Data_Buffer[4], SHT30_Data_Buffer[5],
            last_valid_temp, last_valid_humi);
    }
}

void bh1750_read_data(double *dat)
{
    uint8_t send_data[1] = {0x10};
    uint32_t send_len = 1;

    IoTI2cWrite(I2C_HANDLE, BH1750_I2C_ADDRESS, send_data, send_len);

    uint8_t recv_data[2] = {0};
    uint32_t receive_len = 2;

    IoTI2cRead(I2C_HANDLE, BH1750_I2C_ADDRESS, recv_data, receive_len);
    *dat = (float)(((recv_data[0] << 8) + recv_data[1]) / 1.2);
}

void i2c_dev_init(void)
{
    IoTI2cInit(I2C_HANDLE, EI2C_FRE_400K);
    sht30_init();
    bh1750_init();
}
